                                    ...
                                    <tbody>
                                        @foreach ($pelatihan as $item)
                                            <tr>
                                                ...
                                                <td>
                                                    @php
                                                        $status = isset($item->team->transaction->status)
                                                            ? $item->team->transaction->status
                                                            : $item->transaction->status;
                                                        $payment_proof = isset($item->team->transaction->payment_proof)
                                                            ? $item->team->transaction->payment_proof
                                                            : (isset($item->transaction->payment_proof)
                                                                ? $item->transaction->payment_proof
                                                                : null);
                                                        $id = isset($item->team->transaction->id)
                                                            ? $item->team->transaction->id
                                                            : $item->transaction->id;
                                                        $pelatihan_user_id_transaction = isset(
                                                            $item->team->transaction->pelatihan_user_id,
                                                        )
                                                            ? $item->team->transaction->pelatihan_user_id
                                                            : $item->transaction->pelatihan_user_id;
                                                    @endphp
                                                    ...
                                                    @elseif(
                                                        ($status == 'pending' && $pelatihan_user_id_transaction == $item->id && $payment_proof == null) ||
                                                            ($status == 'failed' && $pelatihan_user_id_transaction == $item->id))
                                                        <a href="{{ route('transaction.bayar.index', Illuminate\Support\Facades\Crypt::encryptString($id)) }}"
                                                            class="btn btn-warning">Upload Bukti Bayar</a>
                                                    @elseif($status == 'need_confirm' && $payment_proof == null)
                                                        Status pembayaran: <span class="badge badge-warning">Menunggu
                                                            Konfirmasi</span>
                                                    ...
                                                    @elseif($status == 'failed')
                                                        Status pembayaran: <span class="badge badge-danger">Pembayaran
                                                            Gagal</span>
                                                    @else
