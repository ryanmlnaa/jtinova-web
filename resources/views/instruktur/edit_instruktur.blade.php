...
<div class="row">
    <div class="col-lg-12">
        <div class="card mx-5 px-3 py-5">
            ...
            <form action="{{route('Instruktur.update', $data->id_instruktur)}}" method="post" enctype="multipart/form-data">
                @csrf
                @method('patch')
                <div class="form-group">
                  <label for="nip" class="col-form-label">Nama Instruktur</label>
                  <input type="text" class="form-control" name="nama_instruktur" value="{{$data->nama_instruktur}}" id="nip">
              </div>
              <div class="form-group">
          ...
          <div class="form-group">
            ...
            <select class="form-control" name="keahlian" id="kedudukan">
                ...
                <option @if($item == $data->bidang_keahlian) {{"selected"}} @endif>{{$item}}</option>
                @endforeach
            </select>
        </div>
          <div class="form-group">
          ...
          <div class="form-group">
              ...
              <input type="hidden" name="old_file_ktp" value="{{ $data->foto_ktp }}">
              <input type="file" class="form-control" name="foto_ktp" id="foto_profile"
                  onchange="previewImageKTP(this);">
              <img id="gambar-preview-ktp" src="{{ url("foto_ktp_instruktur/". $data->foto_ktp)}}" alt="Gambar Pratinjau"
                  style="max-width: 50%; margin-top:20px; display: block;">
          </div>
          <div class="form-group">
            <label for="foto_profile" class="col-form-label">Portofolio Instruktur</label>
            <input type="hidden" name="old_file_porto" value="{{ $data->portofolio }}">
            <input type="file" class="form-control" name="portofolio" id="foto_profile"
                onchange="previewImagePorto(this);">
            <img id="gambar-preview-portofolio" src="{{ url("portofolio_instruktur/". $data->portofolio)}}" alt="Gambar Pratinjau"
                style="max-width: 50%; margin-top:20px; display: block;">
        </div>
            {{-- <div class="form-group">
